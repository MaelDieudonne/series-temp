...
On écrit une fonction qui élimine automatiquement les coefficients non-significatifs au seuil de 5 %. Pour calculer l'intervalle de confiance, on néglige l'incertitude entourant les coefficients estimés, pour considérer seulement la variance des résidus et des prédictions précédentes. D'après les calculs qui précèdent, l'expression générale de la variance est la suivante (en posant ar0 = 1 et en négligeant les termes connus plutôt qu'estimés).

$$
\mathbb{V}(\hat{X}_{t+h}) = (\hat{ar}_{0}^2+\hat{ar}_{1}^2)\mathbb{V}(\hat{X}_{t+h-1}) + (\hat{ar}_{1}^2+\hat{ar}_{2}^2)\mathbb{V}(\hat{X}_{t+h-2}) + \cdots + (\hat{ar}_{10}^2+\hat{ar}_{11}^2)\mathbb{V}(\hat{X}_{t+h-11}) + \mathbb{V}(\varepsilon_t)
$$
