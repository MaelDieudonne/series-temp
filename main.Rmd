...
```{r}
autoplot(raw_series, xlab = "Dates", ylab = "Valeurs") +
...
On écrit une fonction qui élimine automatiquement les coefficients non-significatifs au seuil de 5 %. Pour calculer l'intervalle de confiance, on néglige l'incertitude entourant les coefficients estimés, pour considérer seulement la variance des résidus et des prédictions précédentes. C'est une pratique habituelle, car la variance des coefficients estimés est généralement très inférieure à celle des résidus. Cette hypothèse n'est pas vérifiée ici. On la maintient tout de même, pour éviter de complexifier de manière déraisonnable les calculs.
...
L'expression générale de la variance est la suivante. Pour l'appliquer, on pose ar0 = 1 et on néglige les termes connus plutôt que prédits lorsque l'horizon *h* est inférieur à 11.
...
forecast <- function(h) {
  ...
  # On itère sur les horizons de prédiction.
  for (j in 1:h) {
    T <- nrow(data)
    ...
    data <<- rbind(
      data,
      data.frame(dates = (max(data$dates)) + (1/12), values = exp(prev), log_values = prev, var = var, CIlow = exp(CIlow), CIup = exp(CIup)))
  }
...
On représente finalement les résultats. Le paramètre *r* permet de choisir la durée représentée, prédictions incluses, soit ici 24 mois d'observations et 6 mois de prédiction. On applique une transformation exponentielle aux prédictions et aux bornes de l'intervalle de confiance pour retrouver des valeurs cohérentes avec la série initiale.
