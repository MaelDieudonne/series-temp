...
On écrit une fonction qui élimine automatiquement les coefficients non-significatifs au seuil de 5 %. Pour calculer l'intervalle de confiance, on néglige l'incertitude entourant les coefficients estimés, pour considérer seulement la variance des résidus.

**Le calcul ci-dessous est probablement faux concernant l'IC, il néglige le fait qu'à partir de l'horizon 2, la valeur prédite apparaît (1+ar1) fois. Il faudrait donc avoir, pour h=2, 1.96\*sqrt((1+ar1\^2)\*sigma2), et les choses deviennent plus compliquées ensuite.**

```{r}
# Il serait plus propre de commencer par déterminer quels coefficients sont significatifs, de générer le vecteur correspondant, puis d'itérer dessus, au lieu de tout retester à chaque fois.
...
forecast <- function(h) {
  for (j in 1:h) {
    ...
    for (i in 1:11) {
      ...
      else {if (j==1) {cat("ar", i, " rejected, p-value = ", pval, "\n", sep="")}}
    }
