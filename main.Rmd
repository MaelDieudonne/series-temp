...
On admet alors, devant ces résultats, que la série transformée est stationnaire.
...
$$
IC_{0.95}(\mathbb{V}[\hat{X}_{t+1}]) = \left[ \hat{X}_{t+1} \pm 1.96\sqrt{\sigma} \right]
$$

$$
IC_{0.95}(\mathbb{V}[\hat{X}_{t+2}]) = \left[ \hat{X}_{t+2} \pm 1.96\frac{\sqrt{\sigma}}{\sqrt{(1+\hat{ar}_{1})^2 + 1}} \right]
$$
...
Nous avons également supposé que $\hat{\mathbb{V}}[\hat{X}_{t+h}]$ suit une loi normale centrée de manière à déterminer la région de confiance de niveau $\alpha$ :

$$
IC_{0.95}(\mathbb{V}[\hat{X}_{t+h}]) = \left[ \hat{X}_{t+h} \pm 1.96\sqrt{\hat{\mathbb{V}}[\hat{X}_{t+h}]} \right]
$$

Ces simplifications permettent d'étendre l'horizon de prédiction au-delà de 2. Elle sont justifiées dans la mesure où $P$ n'est pas très différent de 1. On vérifie en particulier :

$$
P(1) = 1 \\
\hat{ar}_{1} = -0,61193077 \Longrightarrow P(2) = (1+\hat{ar}_{1})^2 + 1 \approx 1,15
$$
...
On procède à une vérification avec la fonction forecast du package éponyme : on retrouve les mêmes coefficients, mais des intervalles de confiance plus étroits. Ceci montre que les simplifications précédentes n'étaient pas déraisonnables, puisqu'elles aboutissent à des prédictions plus conservatrices.
